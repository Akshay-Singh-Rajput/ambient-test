...
AmbientDisplay.displayModes = (function () {
  ...
  function select(items, now, config) {
    ...
    var previewOverride = AmbientDisplay.previewMode && AmbientDisplay.previewMode.getIntentOverride();
    ...
    if (previewOverride === 'prepare' && AmbientDisplay.nightClock.isEnabled(cfg)) {

      modeId = 'prepare';

    } else if (previewOverride === 'bedside' && AmbientDisplay.nightClock.isEnabled(cfg)) {

      modeId = 'bedside';

    } else if (hasCelebration(items, cfg)) {
